...
/* #ifdef SERVO_ENDSTOPS
  ...
  SERIAL_PROTOCOLPGM("\n");
	...
	//if maximum number of probes were completed, use the average of all probes, otherwise the last probe value is used
  if (pb_count != pb_count_max){  //pb_count is set equal to pb_count_max only if tolerances are met.
	measured_z=0;  //initiallize
	for (int avg_loop = 0; avg_loop<pb_count_max; avg_loop++) {
	measured_z=measured_z + pb_count_z[avg_loop];
	}
